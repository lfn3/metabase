...
(api/defendpoint PUT "/:id"
  ...
  (let [pulse-before-update (api/write-check Pulse id)]
    ...
    (collection/check-allowed-to-change-position-in-collection pulse-before-update pulse-updates))
  ;; ok, now update the Pulse
  (pulse/update-pulse! (assoc (select-keys pulse-updates [:name :cards :channels :skip_if_empty :collection_id])
                         :id id))
